...
impl<S: 'static + StateManager + ?Sized> Agent<S> {
    ...
    pub fn new(state_manager: Arc<S>) -> Self {
        let agent_id = generate_run_id();
        Agent { state_manager, agent_id }
    }
    ...
    async fn process_queue(&self, queue: &str) {
        if let Some(task_payload) = self.state_manager.get_work_from_queue(queue).await {
            info!(
                ...
                self.agent_id, task_payload.task_run_id, task_payload.dag_run_id, queue
            );

            // Check if the DAG is still active.
            let dag_status = self.state_manager.load_dag_status(&task_payload.dag_run_id).await;
            ...
            self.state_manager.assign_task_to_agent(&self.agent_id, &assignment).await;
            ...
            if task_status == COMPLETED_STATUS {
                ...
                self.state_manager.remove_task_from_agent(&self.agent_id, &assignment).await;
                return;
            }
    ...
    async fn execute_task(&self, task_payload: &TaskPayload) -> bool {
        info!(
            ...
            task_payload.task_run_id, task_payload.command
        );
        ...
        match cmd.output().await {
            Ok(output) if output.status.success() => {
                info!("Task {} completed successfully", task_payload.task_run_id);
                true
            }
            Ok(output) => {
                error!(
